...
function verif() { // Vérifie si une paire a été faite
	...
	if (paires==7) { //Si on trouve toutes les paires
		clearInterval(timerID);
		document.getElementById("photo").style.display = 'block';
		document.getElementById("photo").style.flexDirection = 'column';
		document.getElementById("photo").innerHTML = '<h1> Vous avez gagné !</h1><br /><div class="bouton"><input type="button" class="restart" value="Rejouer" onClick="window.location.reload()"></div>';
	}	// On demande au chronomètre de s'arrêter. On affiche "Vous avez gagné".
}
